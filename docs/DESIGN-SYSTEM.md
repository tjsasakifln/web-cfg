...
- **Fallback com métricas casadas** (`Archivo Fallback`, mais as variantes `Wide` e `Narrow` que escalam o `size-adjust` pela largura) mantém as quebras de linha estáveis quando a fonte chega tarde: `scripts/site/test_font_fallback_metrics.mjs` mede deriva zero do hero com a woff2 bloqueada.
