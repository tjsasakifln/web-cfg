...
| discovered non-scheduled function routes | versioned portable runtime allowlist | Generated proxy for both legacy and `/api/web` aliases to 127.0.0.1:18100; unknown/scheduled names are excluded |
...
| Gate | Result |
|---|---|
| `npm run test:host-contract` | PASS — 21 tests |
...
- host architecture version: `confenge-nginx-node/v2`;
...
No public Netcup origin or certificate was supplied to this branch. The release
pipeline now consumes every generated snippet and binds runtime/storage/static
identity to the same candidate, but no candidate-vs-baseline or post-DNS report
is claimed until `stage_verify` supplies a private host and a later authorized
public-origin gate passes without `--insecure`.
