...
|---|---|---|---|---|---|
| 1. Dono de construtora privada | Sim: projetos, obra/imóvel e decisões documentadas; para quantidades/custos, vê método, entregáveis e lacunas da base. | Sim para existência, identidade, formação e método; nenhuma capacidade não provada é prometida. | `/quantitativos-orcamento-obras/` quando a decisão é orçamento; triagem para os demais casos. | A rota estreita é canônica para quantitativos/orçamento; `/servicos/` segue como hub, sem doorway. | PASS |
| 2. Coordenador de projetos | Sim: compatibilização, projetos complementares, BIM e orçamento aparecem por problema; só orçamento/quantitativos ganha rota específica nesta release. | Sim: método e limites antes do contato. | Rota de quantitativos ou `/triagem-tecnica/#projetos`, conforme o trabalho. | As duas rotas têm papéis distintos e não competem com conteúdo B2G. | PASS |
| 3. Arquiteto buscando engenharia complementar | Sim: o shell usa linguagem do trabalho, não núcleos internos. A rota privada não finge cobrir compatibilização completa. | Sim; ART é tratada condicionalmente ao escopo e à jurisdição, sem promessa automática. | `/triagem-tecnica/#projetos`; quantitativos só quando esse for o pedido. | `/servicos/` continua owner do intent amplo. | PASS |
| 4. Incorporadora | Sim: projeto, orçamento, risco e decisão documentada ficam claros. | Sim no nível institucional comprovado; cases/resultados inexistentes não são inventados. | Rota de quantitativos/orçamento ou triagem contextual. | A rota específica é limitada ao trabalho comprovado; demais money pages ficam DEFER. | PASS |
...
7. `/quantitativos-orcamento-obras/` foi promovida como única money page privada desta release, com self-canonical, método, entregáveis, limites e três canais contextualizados; não virou atalho para perícia, SST, inspeção ou compatibilização.
