...
| `npm run test:copy:language` | passou em 249 superfícies públicas |
| `npm run test:copy:core` | passou |
| `npm run test:privacy` | passou |
| `npm run test:html-integrity` | passou em 251 HTMLs |
...
| `npm run audit:axe` | passou com zero violações críticas/sérias no censo configurado; uma ocorrência moderada preexistente em `/ferramentas/checklist-reequilibrio/`, fora do WRITE_SET |
