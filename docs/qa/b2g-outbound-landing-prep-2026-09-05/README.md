...
- Source baseline: `tjsasakifln/web-cfg@dbf931d7b4a1a9fc2aecd6ef84b3a2b7b1706f55` (`origin/main`, MV-09 release #615). The branch was rebased from the retired base `3552cf228424ebb8f34266f671fd80df43d0615c`; #615 changed only the global shell of these routes, so the route-local hunks carry main's contemporary header, nav and footer.
- Outbound authority: [Warmbly PR #267](https://github.com/tjsasakifln/warmbly/pull/267), audited registry head `a4201f2ff3396f3e08030997563ec397b9627df2`, corrected on the successor branch `fix/267-commercial-destination-match-s548af4f2` at `12c6463e6c919f40a95e1057584979c5f170a41d`.
...
Registry conflict, resolved upstream: at the audited Warmbly head both `APOIO_LICITACAO` and `INTELIGENCIA_PNCP` resolved as `EDITAL_OU_PROPOSTA` to the Bid Room anchor, which told a PNCP recipient that a proposal was being prepared for a dispute that may not exist. The Warmbly successor commit splits the two: `APOIO_LICITACAO` keeps the Bid Room, and `INTELIGENCIA_PNCP` gets its own claim key `MERCADO_PUBLICO_OU_RECORTE_PNCP` routed to the already published `/problemas-que-resolvemos/`. No PNCP copy was invented here and this landing was not broadened to absorb the claim.

Anchor placement, corrected against the earlier reading: on `origin/main` the id `quando-nao-contratar` sat on the supplier-scope grid at line 182, **inside a `<details class="offer-detail-disclosure">` that has no `open` attribute**, so the outbound fragment resolved to collapsed content that does not answer the pinned `PublicProofExpected` ("Criterios para decidir se o edital merece equipe, capital e uma operacao de proposta"). Moving the id to the visible decision section is therefore a fix, not a regression, and Warmbly's `Anchor` is left untouched so no new URL is emitted. The not-hire exclusions keep their own address via the new `id="escopo-limites"`, and a route-local assertion now requires the two to live on separate elements — without it, deleting the whole exclusions block leaves every `id`-OR-`data-when-not-hire` gate in the repo green.

Freeze scope: the frozen pillar set is **six** routes, not four — `scripts/bofu_dominance/frozen_specs/constants.py` PILLARS adds `diagnostico-b2g-360` and `diagnostico-pre-licitacao` to the four measurement routes. None of the six is touched by this branch, and `/bid-room-licitacoes-obras/` is not in `FORBIDDEN_RELATIVE_PATHS`, so it is editable.

Disclosure restored: the two `#metodo` rewrites had dropped "Autor institucional: CONFENGE", the route-specific `Fonte:` clauses, the coverage caveats ("o recorte publico mostra o que esta registrado..." and "a ausencia de guia publico neste eixo nao deve ser interpretada como risco zero"), the "Atualizado em" label and the "nao ha segundo revisor nomeado" clause. No authority or copy gate detects any of these — `check_required_slots` returns `[]` identically before and after — so the copy restoration, not a gate, is the remedy.
...
| `/bid-room-licitacoes-obras/#quando-nao-contratar` | `APOIO_LICITACAO`: decide whether to bid and organize the proposal. `INTELIGENCIA_PNCP`: **no longer routed here** — the Warmbly successor gives it its own claim and sends it to `/problemas-que-resolvemos/`. | `MUTABLE_NOW` for the declared `EDITAL_OU_PROPOSTA` claim; PNCP mapping resolved upstream | Good for tender/proposal: decision, consequence, artifact and proof are specific. | **Before:** fragment targeted the supplier-scope grid inside a `<details>` with no `open` attribute, i.e. collapsed content. **After:** visible decision section answers the supported claim in seven ordered roles. | Advance/decline recommendation, responsibility matrix, risk record and final checklist. | Linked synthetic tender-decision example plus recorded method/source. | “Registrar o edital para revisão de encaixe”, with PII-free `cta_click`. | CONFENGE coordinates/reviews; the client decides, signs and submits; no promised win. | Anchor moved to a visible route-local section; exclusions keep their own `id="escopo-limites"`; PNCP now routed elsewhere upstream without broadening this page. |
