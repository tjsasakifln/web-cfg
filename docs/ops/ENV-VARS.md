...
| `LEAD_DELIVERY_TIMEOUT_MS` | opcional | Orçamento total (todas as tentativas, cabeçalhos **e corpo**) de cada canal de entrega pós-persist — Resend, `OPS_WEBHOOK_URL`, ntfy — e do siteverify do Turnstile. Default 5000; faixa 100–30000. Os canais rodam em paralelo; um provedor pendurado vira `delivery.<canal>.status=error` + `reason=timeout` (ou `timeout_after_http` quando houve resposta HTTP de erro antes do prazo; o `http=<último>` só é guardado para o canal e-mail, `delivery.email.http`) sem derrubar o 201. **Aritmética honesta do POST:** siteverify até 5 s + store + handoff Warmbly até `CONFENGE_INBOUND_TIMEOUT_MS` (8 s) + entrega até 5 s + update do store ≈ 18 s no pior caso somado, acima dos 15 s em que o navegador aborta (`js/modules/form.js`). O que fecha o ciclo nesse pior caso não é o 201 chegar a tempo, e sim o reenvio com a mesma chave `fe-<uuid>` receber o recibo já gravado antes do Turnstile (`LEAD-HANDLING.md`, "reenvio e duplicação"). O default do handoff não foi reduzido porque o requeue é manual (`ops requeue_inbound`) e um handoff perdido custa mais que um reenvio do visitante |
