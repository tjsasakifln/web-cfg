...
| `revops:scheduled-daily` (workflow `revops-scheduled.yml` job `daily`, step "Daily scheduled run") | `scripts/revops/scheduled_daily.mjs` | GET public pages and `build-info.json`; POST `/.netlify/functions/lead` twice (authenticated synthetic lead, `X-Confenge-Probe: <LEAD_PROBE_SECRET>`) only when `LEAD_PROBE_SECRET` (>= 32 chars) is present, otherwise the leg is skipped and reported as `BLOCKED_EXTERNAL dependency=LEAD_PROBE_SECRET`; GET ops counters and `gsc_insights` (read-only consumer polarity, no GSC sync here); POST `ops?action=drain_inbound`; POST `ops?action=produce_search_observation`; POST `ops?action=drain_search_observation` | `OPS_TOKEN`/`REVOPS_TOKEN` (Bearer) for ops; `LEAD_PROBE_SECRET` for the lead leg | REAL_MUTATING |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `probe:lead[:prod]` | Prove capture + idempotent replay + Warmbly transport of an authenticated synthetic lead | `LEAD_PROBE_SECRET` (>= 32 chars) sent as `X-Confenge-Probe`; the server only tags a probe when it matches `LEAD_PROBE_SECRET` | `Idempotency-Key: synthetic-probe-<uuid>` (body `idempotency_key` too; random, not a timestamp); second POST must return `idempotent: true` | none required: record persists as `record_kind=synthetic`, excluded from commercial counters | `record_kind: synthetic`, `test_mode: true`, nome `SYNTHETIC-PROBE`, `example.com` e-mail | Transported to Warmbly synchronously as `record_kind=synthetic`; Resend/notify skip `non_real`; no WhatsApp/e-mail to a person | web-cfg inbound (docs/ops/WARMBLY-INBOUND.md) |
| `scheduled_daily.mjs` lead leg | Daily isolated capture probe | `LEAD_PROBE_SECRET` (>= 32 chars) sent as `X-Confenge-Probe`; without it the leg does not POST and is reported as `BLOCKED_EXTERNAL dependency=LEAD_PROBE_SECRET` (the unauthenticated `X-Confenge-Probe: 1` leg was rejected by production Turnstile since 2026-08-24 and retired on 2026-09-18) | `Idempotency-Key: scheduled-probe-<stamp>` | none required: record persists as `record_kind=synthetic`, excluded from commercial counters | `record_kind: synthetic`, `test_mode: true`, `probe+daily-<stamp>@example.com` | server `non_real` skip; probe records never reach commercial totals | RevOps schedule; the GitHub secret is an external action (docs/ops/EXTERNAL-ACTIONS.md §1) |
