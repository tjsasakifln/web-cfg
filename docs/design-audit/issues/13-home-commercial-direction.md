...
**Nenhum arquivo de fonte novo.** O bloco `@font-face` é byte-idêntico ao anterior: mesmo `assets/archivo-var-latin-b19be0f7.woff2`, mesmos eixos, mesmo subconjunto. O orçamento de `data/site/design-system.json` (`font_files_max: 1`, `font_total_gzip_kb_max: 60`) e os tetos de módulo em `scripts/site/audit_performance.py` não se movem, e o gate `test_font_glyph_coverage.py` continua respondendo pela cobertura de glifos. O eixo de largura é gratuito: já estava no arquivo pago em #526 e simplesmente não estava sendo usado.
