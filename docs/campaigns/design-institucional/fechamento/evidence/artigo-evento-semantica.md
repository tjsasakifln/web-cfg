...
1. A série "formulário a partir de artigo" muda de nome: deixa de existir como `service_cta_click`/`form` na família `/conteudos/` e passa a `content_to_service`/`service`. Leituras que somem `service_cta_click` por origem editorial verão a série cair a zero a partir da publicação; a contagem correta passa a ser `content_to_service` filtrado por `cta_label = 'Continuar pelo formulário'` (o botão do artigo não carrega `data-cta-position`, logo emite `cta_position: 'inline'`; verificado na borda em 2026-09-18, `producao/event-semantics-borda-4cfa6adca.json`).
2. Cada artigo passa a emitir dois `content_to_service` para o mesmo `destination_path`: o botão do formulário (rótulo `Continuar pelo formulário`) e o novo `text-link` do bloco de oferta (rótulo do cluster); ambos sem `data-cta-position`, portanto `cta_position: 'inline'` nos dois. A contagem de `content_to_service` por artigo dobra em potencial; a desambiguação é só por `cta_label`.
