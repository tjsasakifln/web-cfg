...
| --- | --- | --- |
| R1 | hero: "Serviço de engenharia contratado, com responsável técnico: não é software, planilha gratuita nem execução de obra." | fica no hero (literais cobrados por teste); "não executa a obra" reaparece uma única vez em Condições e limites, item 3. Superado em 2026-09-18 (CONFENGE-LAPIDACAO-COMERCIAL-20260918): o hero afirma "Serviço de engenharia contratado, com responsável técnico nomeado."; a lista negativa saiu. Proteção equivalente: `declaresContractedService` em `tests/campaigns/orc-b2b-20260913/test_purchase_path.mjs` exige a proposição afirmativa (serviço contratado + responsável técnico), com controle positivo de duas redações e contraprova sem a proposição |
...
| R4 | `#amostra-quantitativos`: "Não é obra de cliente" (figcaption e `dl.keys`) | fica com a figura (rótulo da amostra). Superado em 2026-09-18 (CONFENGE-LAPIDACAO-COMERCIAL-20260918): a figura leva só o rótulo "Exemplo demonstrativo"; a linha `dt Cliente` do `dl.keys` saiu. Proteção equivalente: `tests/pos-inb-20260911/02/test_purchase_proof.mjs` exige o rótulo visível e "Quantidade não é preço" na página |
| R5 | `#amostra-quantitativos`: "Quantidade não é preço: os valores do recorte são hipotéticos e não representam preço da CONFENGE nem referência SINAPI real." | fica como legenda única da amostra ("nem preço da CONFENGE" é literal cobrado); não se repete fora dos blocos gerados. Superado em 2026-09-18 (CONFENGE-LAPIDACAO-COMERCIAL-20260918): a legenda mantém só "Quantidade não é preço."; a qualificação de preço hipotético vive uma vez, junto da tabela de preços de `/casos/demonstrativo-projeto-privado/#orcamento`. Proteção equivalente: `priceTableQualifiesHypotheticalPrices` + contraprova em `test_purchase_path.mjs` |
| R6 | `#qty-sample-trail` (gerado): "Amostra demonstrativa do recorte de banheiro. Não é orçamento para executar obra, não representa cliente, não é preço da CONFENGE e não é SINAPI real." | fica; pertence ao compositor. Superado em 2026-09-18 (CONFENGE-LAPIDACAO-COMERCIAL-20260918): o descritor (`derive.py` → `consumption.v1.json`) traz `disclaimer` vazio e o compositor não emite o parágrafo; a trilha identifica a fonte no link final. Proteção equivalente: `tests/ativacao_20260912/03/test_presentation_and_sharing.py` exige o link "exemplo demonstrativo do recorte de banheiro" e a ausência de `qty-trail-disclaimer` |
| R7 | `#qty-sample-trail` (gerado): "Preços hipotéticos daquele recorte não são preço do serviço nem SINAPI real." | fica; literal cobrado. Superado em 2026-09-18 (CONFENGE-LAPIDACAO-COMERCIAL-20260918): a frase de preço saiu do link da trilha; a qualificação vive junto da tabela de preços do demonstrativo. Proteção equivalente: `priceTableQualifiesHypotheticalPrices` em `test_purchase_path.mjs` |
| R8 | `#qty-proof-entrances` (gerado, duas vezes): "Este é um exemplo demonstrativo de método. Não representa cliente, obra executada, prazo ou resultado da CONFENGE. Os preços daquele recorte são hipotéticos..." | fica; literal cobrado por entrada. Superado em 2026-09-18 (CONFENGE-LAPIDACAO-COMERCIAL-20260918): cada entrada leva o rótulo visível "Exemplo demonstrativo · Edificação/Infraestrutura" no eyebrow e nenhuma ressalva negativa. Proteção equivalente: `entranceDeclaresDemonstrativeNature` exige o rótulo visível por entrada (atributo, classe ou comentário não contam), com contraprova por contexto: remover o rótulo de uma entrada reprova só aquela |
