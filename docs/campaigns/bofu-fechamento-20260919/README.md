...
| --- | --- | --- |
| IMPLEMENTADO_E_TESTADO | SIM — `site-ci` real da PR #715 (run 35488381533 — site-validation 34 min, pSEO 35488381646; ambos verdes em d28affff0); réplicas locais só como detecção antecipada (ver Publicação e verificação) | PR #715 |
...
- Réplicas locais (detecção antecipada, não autoridade): a de `2c0cecb0b` (`evidence/replica-site-ci-2c0cecb0b.log`) antecipou os três defeitos que o CI real depois confirmou (`?evento=`, altura da home, `test_lighthouse_infra` sob carga); a de `8fa6a661a` (`evidence/replica-site-ci-8fa6a661a.log`, 41 passos gerados do YAML) rodou com um wrapper `if (set -e; eval …)` que **ignora `set -e` dentro da condição do `if`** — os PASS de passos multi-comando (ex.: [09], onde o pytest de `source_contract_hash_drift` reprovou e o passo imprimiu PASS) não são confiáveis, e `8fa6a661a` não é a árvore mesclada (`d28affff0` é). O wrapper foi corrigido depois (`(set -e; …); rc=$?`) e a armadilha está registrada. **Evidência autoritativa:** `site-validation` 35488381533 e pSEO 35488381646 reais em `d28affff0`, `site-ci` de main 35489869473 em `a4871e867` e os gates do SHA exato da release 35489869649.
- Janela da borda com bundle antigo, comprovada benigna (`evidence/janela-borda-bundle-antigo-a4871e867.txt`, 9/9): HTML novo servido com o `script.js` de `fedb4768b` injetado — nenhum erro de página ao escolher "avaliação de imóvel" nem a opção do órgão; botão de envio habilitado e visível; jornada oculta `outro`; formulário válido com o mínimo; `/entregas/` sem erro. Degradação, não quebra de captura.
- Predecessor saudável para reversão: `fedb4768b`.
