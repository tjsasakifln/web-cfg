...
**Wedge:** Pavimentação e edificações públicas no recorte pré-nacional (SC, PI, MG, RS)
...
Wedge sustentável apenas como recorte de 4 UF(s) e 4 mercado(s) publicados. O manifest declara cobertura incompleta frente ao conjunto de referência da base canônica de contratos. PUBLISH exige o export versionado public-read-research-flagship/1.0 com national_claim_allowed, cobertura de 27 UFs, denominator explícito e freshness no SLA. Bloqueio: RESEARCH_READ_MODEL_ABSENT.

**Próxima ação:** Bloqueio RESEARCH_READ_MODEL_ABSENT. Obter o export versionado `public-read-research-flagship/1.0` com national_claim_allowed, denominador nacional fechado e freshness dentro do SLA (48h). Regenerar o pack e só então reavaliar PUBLISH. Não indexar, não promover sitemap, não disparar imprensa.
