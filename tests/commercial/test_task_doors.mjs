...
// 2026-09-10: eram 3 grupos. Inspecao, pericia/avaliacao e seguranca do trabalho
// dividiam um so bloco, entao a diferenca entre os servicos nao era inequivoca.
// Cada um passou a ter bloco, esquema e destino proprios; as exigencias de
// substancia (titulo, paragrafo com corpo e link de saida) valem para os cinco.
assert("engineering_deliveries_have_substance", engineeringGroupHtml.length === 5 && engineeringGroupHtml.every((group) => /<h3>[^<]{12,}<\/h3>/.test(group) && /<p>[^<]{80,}<\/p>/.test(group) && /<a href="\/[^"]+">/.test(group)) && [/projet|compatibiliza/, /quantitativ|orçamento/, /inspe[çc][ãa]o/, /per[íi]cia|avalia[çc][ãa]o/, /seguran[çc]a do trabalho/].every((concept) => concept.test(engineeringGroupText)) && ["/servicos/#servico-projeto", "/quantitativos-orcamento-obras/", "/inspecao-diagnostico-edificacoes/", "/assistencia-tecnica-pericial-engenharia/", "/seguranca-trabalho-apoio-tecnico/"].every((href) => entregas.includes(`href="${href}"`)), "engineering groups");
