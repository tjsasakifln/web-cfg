...
assert("catalog_has_progressive_comparison", !entregas.includes("data-compare-item") && entregas.includes("data-compare-tray") && entregas.includes("data-comparison-items") && catalogScript.includes("cards.forEach(enhanceCard)") && catalogScript.includes('checkbox.dataset.compareItem = ""'), "comparison");
...
assert("catalog_contracts_hydrate_only_on_disclosure", catalogScript.includes('document.addEventListener("toggle"') && catalogScript.includes("hydrateContract(details)") && catalogScript.includes("recordFor(card)?.contractHtml"), "lazy copy contract");
