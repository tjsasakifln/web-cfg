...
## SHA alignment (verifier), non-redeploying freeze

Live deploy identity is **only** `https://confenge.com.br/.well-known/pseo-build.json` (generated by Netlify `npm run build:site`, gitignored).

Freeze without tip chase:

```bash
npm run pseo:evidence:freeze-shas
# commits only seo/pseo-operational-result.json (+ FINAL/audit docs)
# netlify.toml [build].ignore skips publish for evidence-only paths
```

`web_cfg_sha` and `netlify_deployed_sha` are always the live well-known SHA.
`sha_alignment.all_match` is true only when `git rev-parse HEAD` equals that live SHA at freeze time.
