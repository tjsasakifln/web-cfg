...
Both `web_cfg_sha` and `netlify_deployed_sha` in `seo/pseo-operational-result.json` are set from live production:

`curl -sS https://confenge.com.br/.well-known/pseo-build.json` → `409f12984129418e83e627ecaf21e79744b86e9f`

The file `/.well-known/pseo-build.json` is **gitignored** and generated by `npm run build:site` on Netlify each deploy (avoids self-hash commit loops). Production is the source of truth for deploy identity.
