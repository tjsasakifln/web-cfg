...
def render_report(status: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"**Time to evidence:** this PR for the ledger; GSC live remains blocked  ",
        ...
        f"- last_sync error: `{live.get('last_sync_error')}`",
        f"- ready_for_product_decisions: `{live.get('ready_for_product_decisions')}`",
        "",
