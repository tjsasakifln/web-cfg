...
def _assert_decision_nav_names_match_visible_text(html: str) -> None:
    ...
    ordinal is plain visible text (no aria-hidden: what a sighted visitor sees
    and what a voice-control user says are the same string), and a space
    follows it so text runs concatenate the way axe's
    label-content-name-mismatch reads them (BOFU-FECHAMENTO-20260919)."""
    ...
    for attrs, inner in links:
        ...
        assert f"<span>{number}</span> " in inner, (number, inner)
        assert "aria-hidden" not in inner, (number, inner)
        rendered = _visible_text(inner)
