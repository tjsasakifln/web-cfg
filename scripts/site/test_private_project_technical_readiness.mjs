...
expect("fixture_taxonomy", COORDINATION_FIXTURE.taxonomy === "CONFENGE_CORPORATE_TAXONOMY/1.0.0-draft.20260904");
expect("fixture_intake", COORDINATION_FIXTURE.web_intake === "CONFENGE_WEB_INTAKE/2.0.0-draft.20260904");
expect("fixture_hash_present", /^[0-9a-f]{8}$/.test(COORDINATION_FIXTURE_HASH));
...
{
  const first = diagnose(presentAnswers(), { expected_contract_hash: COORDINATION_FIXTURE_HASH, expected_engine_id: ENGINE_ID });
  const second = diagnose(presentAnswers(), { expected_contract_hash: COORDINATION_FIXTURE_HASH, expected_engine_id: ENGINE_ID });
...

{
  let threw = false;
  try {
    diagnose(presentAnswers(), { require_contract_hash: true });
  } catch (err) {
    threw = String(err.message) === "coordination_contract_hash_missing";
  }
  expect("fail_closed_missing_hash", threw);
}

{
  let threw = false;
  try {
    diagnose(presentAnswers(), { expected_contract_hash: "deadbeef" });
  } catch (err) {
    threw = String(err.message) === "coordination_contract_hash_mismatch";
  }
  expect("fail_closed_hash_mismatch", threw);
}
...

{
  ...
  expect("analytics_keys", JSON.stringify(Object.keys(event).sort()) === JSON.stringify(["gap_count", "present_count", "tool", "unknown_count"]));
