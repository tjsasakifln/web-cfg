...
def test_rendered_conflitos_covers_nuclei_min_data_and_fail_closed_copy():
    ...
    assert "cliente privado" in lower or "clientes privados" in lower
    ...
    assert re.search(r"\bcorpus\b", vis, flags=re.I) is None
    assert re.search(r"\brollback\b", vis, flags=re.I) is None
    assert "CONFENGE_PUBLIC_CONFLICT_GATE" not in article
    assert "same_public_duty_matter" not in vis
