...
def test_declared_family_parent_route_answers_with_a_page_not_a_server_error():
    """/oportunidades/ is a declared family prefix and must not 403.

    The directory holds four published opportunity pages. With no `index.html`
    and nginx `autoindex off`, the parent route answered a raw
    `403 Forbidden` — an unbranded server error where the family's front door
    should be. The page is generated by
    `scripts.live_intelligence.render.render_opportunities_index_html`; this
    gate only checks the published result, so a regeneration that dropped it
    would be caught here too.

    It stays out of the index surface while its children are noindex: the
    parent must never become an indexable pointer at noindex routes.
    """
    registry = json.loads(
        (ROOT / "data/organic/public-family-registry.json").read_text(encoding="utf-8")
    )
    prefixes = {
        str((f.get("match") or {}).get("prefix") or "")
        for f in registry.get("families") or []
    }
    assert "/oportunidades/" in prefixes, "family declaration disappeared"

    index_page = ROOT / "oportunidades" / "index.html"
    assert index_page.is_file(), "/oportunidades/ has no index.html and would 403"
    html = index_page.read_text(encoding="utf-8")
    assert is_noindex(html)
    assert "<h1>" in html and "site-header" in html, "the page must be branded"

    children = sorted(
        child
        for child in (ROOT / "oportunidades").iterdir()
        if child.is_dir() and (child / "index.html").is_file()
    )
    assert children, "the family directory must still hold opportunity pages"
    for child in children:
        assert f'href="/oportunidades/{child.name}/"' in html, child.name
