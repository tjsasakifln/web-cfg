...
def test_checked_in_canary_matches_current_approval_decision(monkeypatch) -> None:
    ...
    assert "Autoria: <a rel=\"author\"" in expected_html
    assert "Revisão técnica:" not in expected_html
    assert "segundo revisor" not in expected_html.lower()
    ...
    assert "approval_material_hash_mismatch" not in status["recommendation_reason"]
