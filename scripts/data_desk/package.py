...
def press_brief(asset: dict[str, Any], package: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {asset.get('title')}",
        "",
        f"**Marca d'água:** {watermark}" if watermark else "",
        f"**Identificador:** {package.get('identifier')}",
        f"**Dados consultados em:** {package.get('as_of')}",
        f"**Método / esquema / versão dos dados:** {package.get('method_version')} / {package.get('schema_version')} / {package.get('data_version')}",
        f"**Link permanente:** {package.get('permalink')}",
        f"**Fonte canônica pública:** {package.get('canonical') or '(não se aplica a este material interno)'}",
        "",
        "## Citação sugerida",
        "",
        ...
        "",
        asset.get("can_answer") or "Somente as perguntas descritas no registro deste material.",
        "",
        "## O que estes dados não respondem",
        "",
        asset.get("cannot_answer") or "Questões fora da cobertura e das limitações declaradas.",
        "",
        "## Correções",
        "",
        f"Canal de correção: {package.get('correction_link')}",
        f"Responsável: {package.get('correction_owner') or asset.get('correction_owner')}",
        "",
        "## Contato",
        "",
        ...
        "## Condições de uso",
        "",
        package.get("license_notice") or "Consulte os Termos de Uso da CONFENGE.",
        "",
...
def build_package(
    ...
    *,
    ...
    package: dict[str, Any] = {
        ...
        "license_notice": asset.get("license_notice"),
