...
<ol><li><strong>Uma unidade basta</strong><span>Quando existe uma pergunta delimitada e o recorte permanece em uma empresa, território, tipologia, período ou módulo declarado na oferta.</span></li>
<li><strong>Diagnóstico integrado</strong><span>Quando ${escapeHtml(ladder.diagnosis_trigger)}, o <a href="${escapeHtml(ladder.diagnosis_route)}">${escapeHtml(pkg.public_name_pt_br)}</a> reúne ${escapeHtml(ladder.diagnosis_scope)} por ${escapeHtml(pkg.package_price_display)}. O maior valor pago em uma única unidade 02 a 08 é abatido se ele for contratado em até ${pkg.credit_window_days} dias, sem acúmulo. A unidade 01 permanece fora do Diagnóstico e não gera crédito de ${pkg.credit_window_days} dias.</span></li>
