...
function vitrineCard(entry, contractItem) {
  ...
  // __actions, ordem entrega -> preco) continuam identicos; muda a composicao.
  const open = `<li><span class="list-ruled__index">${entry.catalog_number}</span><article class="vitrine-item${anchorClass}" data-primary-offer="true" data-deliverable-id="${entry.deliverable_id}" data-public-state="${entry.public_state}" data-search-aliases="${searchAliases(entry)}" id="entrega-${entry.catalog_number}">`;
  const close = "</article></li>";
  return `${open}
<header class="vitrine-item__head"><h2 id="${headingId}">${escapeHtml(entry.public_name_pt_br)}</h2><p class="vitrine-item__price"><span>Preço</span><strong>${escapeHtml(priceLabel(entry))}</strong></p></header>
...
<header class="sec-head sec-head--split"><span class="t-kicker">Próxima camada pelo tipo de decisão</span><h3 class="t-editorial" id="offer-value-ladder-title">Unidade, Diagnóstico ou direção recorrente?</h3></header>
...
<div><dt>As sete unidades, uma a uma</dt><dd class="t-data">${escapeHtml(pkg.units_sum_display)}</dd></div>
<div><dt>${escapeHtml(pkg.public_name_pt_br)}</dt><dd class="t-data">${escapeHtml(pkg.package_price_display)}</dd></div>
</dl>
<p class="compare-note t-caption">Sete ofertas geram um único crédito, sem acúmulo, se o ${escapeHtml(pkg.public_name_pt_br)} for contratado em até ${pkg.credit_window_days} dias. A unidade 01 fica fora do Diagnóstico e não gera crédito de ${pkg.credit_window_days} dias. A diferença entre ${escapeHtml(pkg.units_sum_display)} e ${escapeHtml(pkg.package_price_display)} é R$ 4.280.</p>
</section>`;
}
...
function renderOfferShowcase(published, eightContract) {
  ...
  return `<section class="deliverables-vitrine sec sec--soft" id="enquadrar" data-section-archetype="catalog_index" aria-labelledby="catalog-title">
<div class="container">
<header class="deliverables-vitrine__intro sec-head sec-head--split"><p class="eyebrow t-kicker">Ofertas com preço publicado</p><h2 class="t-editorial" id="catalog-title">Oito análises para licitações e mercado de obras públicas.</h2><p>Os demais serviços de engenharia seguem por proposta porque variam com disciplina, fase, documentos e responsabilidade.</p></header>
${renderDecisionNav(published)}
<div class="vitrine-items"><ol class="list-ruled list-ruled--offers">
${published.map((entry) => vitrineCard(entry, contractById.get(entry.deliverable_id))).join("\n")}
</ol></div>
${renderOfferLadder(eightContract)}
</div>
</section>
<section class="sec sec--tight" id="condicoes-e-limites" data-section-archetype="limitation_notice" aria-labelledby="published-common-title">
<div class="container">
<div class="published-offers__common"><div class="conditions"><span class="t-kicker">Condições da análise</span><h2 class="t-editorial" id="published-common-title">Condições e limites: o que vale para as oito ofertas</h2>
<ul>
...
<li><span class="list-ruled__index">01</span><article class="capability-group"><h3>Projetos, revisão e compatibilização</h3><p>Cálculos, plantas, detalhes, especificações e memória de cálculo; relatório de revisão com pontos localizados; ou registro das interferências e soluções entre estruturas e instalações hidrossanitárias, elétricas, de incêndio, climatização e telecomunicações. Serve para completar o projeto ou conferir o de terceiro antes de executar.</p><div class="capability-group__schema"><details class="capability-group__schema-details"><summary class="capability-group__schema-title"><strong>Esquema ilustrativo da entrega.</strong> Como o conteúdo do projeto se organiza</summary><p class="capability-group__schema-label">Para que serve cada peça.</p><ol class="capability-group__schema-list"><li><strong>Plantas e cortes</strong> onde cada elemento fica e como a obra se lê no papel.</li><li><strong>Detalhes</strong> as ligações e os pontos que a planta não consegue mostrar na mesma escala.</li><li><strong>Especificações</strong> o que cada material e serviço precisa atender para ser aceito.</li><li><strong>Memória de cálculo</strong> os critérios e as verificações que sustentam o que foi dimensionado.</li><li><strong>Registro de compatibilização</strong> as interferências encontradas entre disciplinas e a solução acordada para cada uma.</li></ol><p class="capability-group__schema-note">Cada peça responde a uma pergunta diferente na obra: a planta orienta a execução, o detalhe resolve a ligação, a especificação define o aceite e a memória sustenta a decisão técnica.</p></details></div><p class="capability-group__link"><a href="/servicos/#servico-projeto">Entender o trabalho de projeto <svg class="icon"><use href="#i-arrow"></use></svg></a></p></article></li>
<li><span class="list-ruled__index">02</span><article class="capability-group"><h3>Quantitativos e orçamento</h3><p>Planilha de serviços e quantidades, memória dos critérios, composições abertas, referências e data-base para comparar propostas, preparar contratação ou revisar a estimativa de obra pública ou privada.</p><div class="capability-group__schema"><details class="capability-group__schema-details"><summary class="capability-group__schema-title"><strong>Esquema ilustrativo da entrega.</strong> Como o orçamento se forma</summary><p class="capability-group__schema-label">Para que serve cada peça.</p><ol class="capability-group__schema-list"><li><strong>Levantamento de quantidades</strong> o que será executado e quanto, item a item, com o critério de medição declarado.</li><li><strong>Composições de custo</strong> mão de obra, material e equipamento de cada serviço, abertos para conferência.</li><li><strong>Referências e data-base</strong> a fonte de preço adotada e a data a que ela se refere.</li><li><strong>Planilha orçamentária</strong> o total por item e por etapa, na estrutura em que a proposta será comparada.</li><li><strong>Memória dos critérios</strong> o que foi incluído, o que foi excluído e por quê.</li></ol><p class="capability-group__schema-note">É essa cadeia que permite comparar propostas pelo mesmo critério, em vez de comparar apenas o preço final.</p></details></div><p class="capability-group__link"><a href="/quantitativos-orcamento-obras/">Ver quantitativos e orçamento de obras <svg class="icon"><use href="#i-arrow"></use></svg></a></p></article></li>
<li><span class="list-ruled__index">03</span><article class="capability-group"><h3>Inspeção e diagnóstico de edificações</h3><p>Relatório de inspeção ou laudo de estado, com localização das evidências e a condição encontrada. Serve para priorizar manutenção, orçar reparo, decidir a intervenção, receber ou entregar a obra com a lista do que corrigir, aprovar uma reforma em condomínio ou documentar o que foi construído.</p><div class="capability-group__schema"><details class="capability-group__schema-details"><summary class="capability-group__schema-title"><strong>Esquema ilustrativo da entrega.</strong> Como o relatório de inspeção se organiza</summary><p class="capability-group__schema-label">Para que serve cada peça.</p><ol class="capability-group__schema-list"><li><strong>Objeto e acesso</strong> o que foi inspecionado, em que data e o que pôde ser visto no local.</li><li><strong>Manifestações registradas</strong> onde cada fissura, infiltração ou desgaste aparece, com localização.</li><li><strong>Origem provável</strong> a explicação técnica compatível com o que foi observado.</li><li><strong>Recomendação</strong> o que fazer, em que ordem, e o que precisa ser investigado antes.</li><li><strong>Limites do exame</strong> o que não foi acessível e o que dependeria de ensaio ou abertura.</li></ol><p class="capability-group__schema-note">A inspeção descreve a condição do que existe. Ela orienta manutenção e reparo, e não substitui a perícia quando a pergunta é de responsabilidade ou de prova.</p></details></div><p class="capability-group__link"><a href="/inspecao-diagnostico-edificacoes/">Ver inspeção e diagnóstico de edificações <svg class="icon"><use href="#i-arrow"></use></svg></a> <a href="/inspecao-diagnostico-edificacoes/#recebimento-entrega">Recebimento ou entrega <svg class="icon"><use href="#i-arrow"></use></svg></a> <a href="/inspecao-diagnostico-edificacoes/#reforma-condominio">Reforma em condomínio <svg class="icon"><use href="#i-arrow"></use></svg></a> <a href="/inspecao-diagnostico-edificacoes/#documentacao-as-built">Documentação do construído <svg class="icon"><use href="#i-arrow"></use></svg></a></p></article></li>
<li><span class="list-ruled__index">04</span><article class="capability-group"><h3>Perícia, assistência técnica e avaliação</h3><p>Manifestação técnica da parte, com evidências organizadas, quesitos e crítica de laudo de terceiro, ou laudo de avaliação de imóvel, com método, evidências e conclusão delimitada. Serve para sustentar a posição técnica da parte ou fundamentar o valor de um imóvel.</p><div class="capability-group__schema"><details class="capability-group__schema-details"><summary class="capability-group__schema-title"><strong>Esquema ilustrativo da entrega.</strong> Como o laudo, o parecer ou a avaliação se estrutura</summary><p class="capability-group__schema-label">Para que serve cada peça.</p><ol class="capability-group__schema-list"><li><strong>Objeto e quesitos</strong> o que foi examinado e as perguntas que o documento precisa responder.</li><li><strong>Evidências</strong> o que foi observado, medido ou obtido em documento, com data e origem.</li><li><strong>Método</strong> como as evidências foram analisadas e qual norma ou referência foi aplicada.</li><li><strong>Conclusão</strong> a resposta aos quesitos, delimitada ao que as evidências sustentam.</li><li><strong>Limites</strong> o que o documento não conclui e o que exigiria investigação adicional.</li></ol><p class="capability-group__schema-note">Diferente da inspeção, aqui a pergunta é de prova ou de valor: o método declarado e a rastreabilidade das evidências são o que permite a terceiros conferir a conclusão. O papel técnico permanece separado do trabalho jurídico.</p></details></div><p class="capability-group__link"><a href="/assistencia-tecnica-pericial-engenharia/">Ver assistência técnica pericial <svg class="icon"><use href="#i-arrow"></use></svg></a> <a href="/servicos/#servico-avaliacao">Ver avaliação de imóvel <svg class="icon"><use href="#i-arrow"></use></svg></a></p></article></li>
<li><span class="list-ruled__index">05</span><article class="capability-group"><h3>Segurança do trabalho</h3><p>Programa de gerenciamento de risco, laudo de exposição ou parecer sobre a frente de obra, dentro da atribuição profissional confirmada. Serve para atender a exigência do contratante.</p><div class="capability-group__schema"><details class="capability-group__schema-details"><summary class="capability-group__schema-title"><strong>Esquema ilustrativo da entrega.</strong> Como o documento de segurança do trabalho se organiza</summary><p class="capability-group__schema-label">Para que serve cada peça.</p><ol class="capability-group__schema-list"><li><strong>Atividades e frentes</strong> o que é executado, por quem e em que condição.</li><li><strong>Agentes e exposição</strong> os riscos identificados e como a exposição foi caracterizada.</li><li><strong>Controles existentes</strong> o que já está implantado e o que a documentação interna registra.</li><li><strong>Medidas e prioridade</strong> o que corrigir, em que ordem, e o que depende de terceiros.</li><li><strong>Registros</strong> o que precisa ser mantido e revisado, e em que periodicidade.</li></ol><p class="capability-group__schema-note">O documento serve para corrigir controles, manter registros e subsidiar a assistência técnica. Atribuições de engenharia não se confundem com atos médicos.</p></details></div><p class="capability-group__link"><a href="/seguranca-trabalho-apoio-tecnico/">Ver apoio técnico de SST <svg class="icon"><use href="#i-arrow"></use></svg></a></p></article></li>
</ol>
<p class="catalog-publication-dates t-caption"><small>Publicação original: <time datetime="${dates.date_published}">${datePtBr(dates.date_published)}</time>. Revisão comercial: <time datetime="${dates.date_modified}">${datePtBr(dates.date_modified)}</time>.</small></p>
</div>
</section>`;
}
