...
function ensureCss(html) {
  const link = '<link href="/assets/contract-defense-products.css" rel="stylesheet"/>';
  return html.includes(link) ? html : html.replace("</head>", `${link}\n</head>`);
}
...
function routeBlock(item, hasForm) {
  ...
  return `${ROUTE_START}
<section class="contract-product" aria-labelledby="contract-product-${item.item}"><div class="container"><header><p class="eyebrow">Unidade ${item.item} · preço-piloto</p><h2 id="contract-product-${item.item}">${esc(item.public_name_pt_br)}</h2><p>${esc(item.value_line_pt_br)}</p></header><dl class="contract-product__lockup"><div><dt>Preço-piloto</dt><dd>${price(item.pilot_price_cents)}</dd></div><div><dt>SLA</dt><dd>${item.sla_business_days} dias úteis</dd></div><div><dt>Unidade</dt><dd>${esc(item.scope_unit_pt_br)}</dd></div></dl><div class="contract-product__grid"><section><h3>Documento mínimo</h3><p>${esc(item.minimum_document_pt_br)}</p></section><section><h3>Saída</h3><p>${esc(publicEvidence(item.output_pt_br))}</p></section><section><h3>Não inclui</h3>${list(item.not_included_pt_br)}</section><section><h3>Prazo seguro</h3><p>${esc(gate)}</p><p>${esc(item.legal_boundary.statement_pt_br)}</p></section></div><article class="contract-product__example" aria-label="Exemplo sintético do método"><p data-evidence-grade="EVENT"><strong>Evento sintético</strong> Uma divergência é registrada no contrato demonstrativo.</p><p data-evidence-grade="FACT"><strong>Prova · Fato</strong> ${esc(publicEvidence(item.evidence_grades.FACT))}</p><p data-evidence-grade="CALCULATION"><strong>Cálculo</strong> ${esc(publicEvidence(item.evidence_grades.CALCULATION))}</p><p data-evidence-grade="INFERENCE"><strong>Leitura · Inferência</strong> ${esc(publicEvidence(item.evidence_grades.INFERENCE))}</p><p data-evidence-grade="UNKNOWN"><strong>Lacuna · Não informado</strong> ${esc(publicEvidence(item.evidence_grades.UNKNOWN))}</p><p data-evidence-grade="DECISION"><strong>Decisão</strong> ${esc(item.decision_question_pt_br)}</p></article>${hasForm ? '<p class="contract-product__action"><a class="button button-primary" href="#captura-pilar">Registrar este evento</a></p>' : standaloneForm(item)}</div></section>
${ROUTE_END}`;
}

function hubBlock() {
  const cards = contract.items.map((item) => `<article class="contract-products-hub__card" id="entrega-${item.item}"><p>${String(item.item).padStart(2, "0")}</p><h3>${esc(item.public_name_pt_br)}</h3><p>${esc(item.value_line_pt_br)}</p><dl><div><dt>Preço</dt><dd>${price(item.pilot_price_cents)}</dd></div><div><dt>SLA</dt><dd>${item.sla_business_days} dias úteis</dd></div></dl>${item.route ? `<a href="${item.route}">Ver escopo e exemplo</a>` : '<a href="#captura-contrato">Registrar necessidade de reajuste</a>'}</article>`).join("");
  return `${HUB_START}
<section class="contract-products-hub" aria-labelledby="contract-products-title"><div class="container"><header><p class="eyebrow">Proteger caixa, prazo e margem</p><h2 id="contract-products-title">Sete decisões contratuais, com limite antes da proposta.</h2><p>Escolha pelo evento. Fontes públicas complementam documentos contemporâneos; cada entrega é obrigação de meio e a estratégia jurídica permanece com o cliente.</p></header><aside class="contract-products-hub__rules" aria-label="Crédito, urgência e limites"><p>${esc(contract.common_rules.credit_rule.statement_pt_br)}</p><p>${esc(contract.common_rules.urgency_rule.statement_pt_br)}</p><p>${esc(contract.common_rules.public_sources_rule.statement_pt_br)}</p><p>${esc(contract.common_rules.obligation_rule.statement_pt_br)}</p></aside><div class="contract-products-hub__grid">${cards}</div>${standaloneForm(null, { hub: true })}</div></section>
${HUB_END}`;
}
...
if (process.argv.includes("--check")) {
  ...
  console.log(`CONTRACT_DEFENSE_PRODUCTS_OK routes=${updates.length - 1} hub=1`);
} else if (process.argv.includes("--write")) {
  for (const { absolute, next } of updates) fs.writeFileSync(absolute, next);
  console.log(`CONTRACT_DEFENSE_PRODUCTS_WRITTEN routes=${updates.length - 1} hub=1`);
} else {
