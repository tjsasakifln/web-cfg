...
function hubBlock() {
  const cards = contract.items.map((item) => {
    ...
    return `<article class="contract-products-hub__card" id="entrega-${item.item}"><p>${String(item.item).padStart(2, "0")}</p><h3>${esc(item.public_name_pt_br)}</h3><p>${esc(item.value_line_pt_br)}</p><dl><div><dt>Preço</dt><dd>${price(item.pilot_price_cents)}</dd></div><div><dt>SLA</dt><dd>${item.sla_business_days} dias úteis</dd></div></dl>${action}</article>`;
  }).join("");
  return `${HUB_START}
