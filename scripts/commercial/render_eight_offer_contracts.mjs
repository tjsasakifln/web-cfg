...
<div class="container"><header class="eight-contract__head"><p class="eyebrow">Decisão e entrega desta unidade</p><h2 id="eight-contract-${item.number}">${escapeHtml(item.issue_331_name)}</h2><p data-copy-role="value_outcome">${escapeHtml(value.actual_contract_value)}</p></header>
<div class="eight-contract__scope eight-contract__value"><section data-copy-role="value_created"><h3>Trabalho que a entrega comprime</h3><p>${escapeHtml(value.work_removed)}</p></section><section data-copy-role="artifact"><h3>Como a entrega entra na decisão</h3><p>${escapeHtml(value.artifact_use)}</p></section><section data-copy-role="positive_proof"><h3>O que você pode inspecionar</h3><p>${escapeHtml(value.proof_statement)}</p></section><section><h3>Por que este preço</h3><p>${escapeHtml(value.price_anchor)}</p></section></div>
...
<dl class="eight-contract__terms"><div><dt>Preço preservado</dt><dd>${escapeHtml(item.price_display)}</dd></div><div><dt>Prazo de entrega</dt><dd>${escapeHtml(item.sla.text)}</dd></div><div><dt>Marco do prazo</dt><dd>${escapeHtml(item.sla.counts_from === "UNKNOWN" ? "confirmado antes da cobrança" : item.sla.counts_from)}</dd></div></dl>
