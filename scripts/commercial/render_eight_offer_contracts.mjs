...
function hubBlock(contract) {
  const cards = contract.deliverables.map((item) => `<article class="eight-hub__item"><header class="eight-hub__item-head"><span>${item.number}</span><strong>${escapeHtml(item.issue_331_name)}</strong><em>${escapeHtml(item.price_display)}</em></header><dl><div><dt>Objeto incluído</dt><dd>${escapeHtml(item.objeto_incluido)}</dd></div><div><dt>Saída mínima</dt><dd>${escapeHtml(item.saida_minima)}</dd></div><div><dt>SLA</dt><dd>${escapeHtml(item.sla.text)}</dd></div></dl><a data-asset-id="entregas-exemplos-hub" data-cta-id="deliverables-hub-scope-${item.number}" data-cta-position="eight_hub_${item.number}" data-event-name="cta_click" href="${item.route}">Abrir ${escapeHtml(item.issue_331_name)}</a></article>`).join("\n");
