...
function hubBlock(contract) {
  const cards = contract.deliverables.map((item) => `<details class="eight-hub__item"><summary><span>${item.number}</span><strong>${escapeHtml(item.issue_331_name)}</strong><em>${escapeHtml(item.price_display)}</em></summary><div><dl><div><dt>Objeto incluído</dt><dd>${escapeHtml(item.objeto_incluido)}</dd></div><div><dt>Saída mínima</dt><dd>${escapeHtml(item.saida_minima)}</dd></div><div><dt>SLA</dt><dd>${escapeHtml(item.sla.text)}</dd></div></dl><section><h3>Entradas</h3>${list(item.entrada)}</section><section><h3>Fronteiras</h3>${list(item.fronteira.map((value) => publicCopy(contract, value)))}</section><a href="${item.route}">Ver exemplo integral e registrar esta unidade</a></div></details>`).join("\n");
  return `${HUB_START}
<section class="eight-hub" data-section-archetype="reading_method" aria-labelledby="eight-hub-title"><div class="container"><header><p class="eyebrow">Escopo antes da contratação</p><h2 id="eight-hub-title">Compare o contrato das oito unidades atuais.</h2><p>Abra cada unidade para conferir objeto, entradas, saída, SLA e fronteiras. Cobertura, data de corte, método e o rótulo NÃO INFORMADO acompanham o resultado; preço e aritmética do pacote permanecem iguais.</p></header><div class="eight-hub__list">${cards}</div></div></section>
${HUB_END}`;
}
