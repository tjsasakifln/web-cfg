...
class _ArticleText(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag == "meta" and ad.get("name", "").lower() == "robots":
...
def measure_cluster(root: Path | None = None) -> dict[str, Any]:
    ...
    for i, ra in enumerate(OWNED_ROUTES):
        for rb in OWNED_ROUTES[i + 1 :]:
            ratio = pairwise_overlap(paragraph_set(pages[ra]), paragraph_set(pages[rb]))
            pairs.append(
                {
                    "a": ra,
                    "b": rb,
                    "shared": sorted(paragraph_set(pages[ra]) & paragraph_set(pages[rb])),
                    "overlap": round(ratio, 4),
                    "size_a": len(paragraph_set(pages[ra])),
                    "size_b": len(paragraph_set(pages[rb])),
                }
            )
    return {
